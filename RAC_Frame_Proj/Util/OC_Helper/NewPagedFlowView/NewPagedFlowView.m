...
- (void)removeCellAtIndex:(NSInteger)index{
    ...
    if (cell.superview  == nil) {
        [cell removeFromSuperview];  //加了ad广告页之后屏蔽掉的
    }
...
- (void)reloadData {
    ...
    if (_needsReload) {
        ...
        //重置pageWidth
        _pageSize = CGSizeMake(self.bounds.size.width - 4 * self.leftRightMargin,(self.bounds.size.width - 4 * self.leftRightMargin) * 0.5);
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    [self startTimer];
    [self setLayerZpositionDidScorll];
}
...
- (void)setLayerZpositionDidScorll {
    
    for (PGIndexBannerSubiew *cell in _cells) {
        if (![cell isKindOfClass:[NSNull class]]) {
            cell.layer.zPosition = 0.0;
        }
    }
    if (self.page < _cells.count) {
        PGIndexBannerSubiew *aCell = [_cells objectAtIndex:self.page];
        if (![aCell isKindOfClass:[NSNull class]]) {
            aCell.layer.zPosition = 1.0;
        }
    }
}

- (void)setLayerZPosition {
    
    NSMutableArray *mutArr = [NSMutableArray arrayWithArray:_cells];
    for (PGIndexBannerSubiew *cell in _cells) {
        if ([cell isKindOfClass:[NSNull class]]) {
            [mutArr removeObject:cell];
        }
    }
    NSInteger idx = mutArr.count/2;
    for (PGIndexBannerSubiew *cell in mutArr) {
        PGIndexBannerSubiew *subCell = [mutArr objectAtIndex:idx];
        if (cell == subCell) {
